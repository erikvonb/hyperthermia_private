...
freq_combs_h = make_combinations_chooser(p_right);
...
function objs = make_combinations_chooser(p)
    make_control(p, 'text', 'Choose frequency combinations to optimize:', ...
        []);
    posvec = [10, ...
              total_height - cumulative_height - 40, ...
              sub_width / 4, ...
              sub_height];
    objs{1} = make_control_no_advancement(p, 'checkbox', 'f1-f2', ...
        posvec, ...
        'value', 1);
    posvec(1) = 10 + (sub_width - 20) / 4;
    objs{2} = make_control_no_advancement(p, 'checkbox', 'f2-f2', ...
        posvec, ...
        'value', 1);
    posvec(1) = 10 + (sub_width - 20) / 2;
    objs{3} = make_control_no_advancement(p, 'checkbox', 'f2-f1', ...
        posvec, ...
        'value', 1);
...
function objs = make_settings_finder(p, content_text, ...
    ...
    objs{3} = make_control_no_advancement(p, 'checkbox', '', posvec_3, ...
    ...
    switch type
        case 'dropdown'
            obj = uidropdown(p, ...
                ...
                'position', posvec);
        case 'text'
            posvec(4) = sub_height * (1 + floor(length(content)/45));
            posvec(2) = posvec(2) - sub_height * floor(length(content)/45);
            obj = uilabel(p, ...
                'text', content, ...
                'position', posvec);
        case 'editnum'
